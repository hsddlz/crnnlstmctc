...
for i in range(len(DF['index'])):
    d_get_label[DF['index'][i]] = i + 11
    d_get_inverse_label[i+11] = unicode(DF['index'][i])

def get_label(st):
    global d_get_label
    return [d_get_label[s] if s in d_get_label else 93 for s in st]
...
def get_padded_label(s,cutoff=22):
    ...
    for i in range(min(len(s),cutoff)):
        if i>0 and beforepad[i]==93 and beforepad[i-1]==93:
            pass
        else:
            curpos+=1
    ...
    def __iter__(self):
        ...
        for k in range(pic_num / self.batch_size):
            ...
            for i in range(self.batch_size):
                ...
                for i in range(rand_1,img.shape[0]):
                    if i-rand_1>=img.shape[0]:
                        break
                    for j in range(rand_2,img.shape[1]):
                        if j-rand_2>=img.shape[1]:
                            break
                        newimg[i-rand_1,j-rand_2] = img[i-rand_1,j-rand_2]
...
def LCS_Purge(p,l):
    mp = [item for item in p if item!=93]
    ml = [item for item in l if item!=93]
